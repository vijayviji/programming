...
void list_insert_at_front(List *list, ListCustomNode *node);
void list_insert_at_rear(List *list, ListCustomNode *node);
void list_init_element(ListCustomNode *node, void *data);
ListCustomNode * list_alloc_element(void *data);
ReturnStatus list_insert_data(List *list, void *data, bool at_front);
void list_item_unlink(ListCustomNode *node);
void list_item_free(ListCustomNode *node, bool destroy_data);
...
ListCustomNode * list_alloc_element(void *data)
{
   ListCustomNode *tmp = (ListCustomNode *) malloc(sizeof(ListCustomNode));
...
ReturnStatus list_insert_data(List *list, void *data, bool at_front)
{
   ListCustomNode *tmp = list_alloc_element(data);
                  ...
                  HTPrintKV print_kv_fn)           // IN
{
   ...
   h_table->stats.collisions = (uint64_t *) malloc(sizeof(uint64_t) * num_buckets);
...
int** palindromePairs(char** words, int wordsSize, int** columnSizes, int* returnSize)
{
   ...
   *returnSize = 0;
   ...
   return_arr = (int **) malloc(sizeof(int *) * 10000);
   assert(return_arr != NULL);

   retColumnSizes = (int *) malloc(sizeof(int) * 10000);
